...
"
    ...
    if [ "$_PKG_OK" -eq 1 ]; then
        ...
        # ---------------------------------------------------------------
        # pfSense repo config — validate and fix URL version mismatch.
        # The native config at /usr/local/share/pfSense/pkg/repos/ may
        # have been tampered with to point at an older version's packages,
        # preventing security updates while appearing functional.
        # Always verify the URL version matches the installed pfSense version.
        # ---------------------------------------------------------------
        _FINGERPRINTS="/usr/local/share/pfSense/keys/pkg"
        _NATIVE_CONF="/usr/local/share/pfSense/pkg/repos/pfSense-repo.conf"

        # Check if native config version matches installed version
        if [ -f "$_NATIVE_CONF" ]; then
            _NATIVE_VER="$(grep 'url:' "$_NATIVE_CONF" 2>/dev/null | \
                head -1 | grep -oE 'v[0-9_]+' | head -1 | tr '_' '.')"
            _NATIVE_VER="${_NATIVE_VER#v}"  # strip leading v
            if [ -n "$_NATIVE_VER" ] && [ "$_NATIVE_VER" != "$_rv" ]; then
                msg_wn "REPO VERSION MISMATCH DETECTED"
                msg_wn "  Installed pfSense : $_rv"
                msg_wn "  Repo config says  : $_NATIVE_VER"
                msg_wn "This may indicate the repo config was tampered with to block updates."
                msg_in "Correcting repo config to match installed version $_rv ..."
                log "ALERT: Repo version mismatch — installed=$_rv repo=$_NATIVE_VER"
            else
                msg_ok "Repo config version matches installed pfSense: $_rv"
            fi
        fi

        # Always write correct URLs based on installed version, not native config
        ...
        # Also fix the native config so pfSense's own updater uses correct URLs
        printf 'FreeBSD: { enabled: no }\n\npfSense-core: {\n  url: "%s",\n  mirror_type: "srv",\n  signature_type: "fingerprints",\n  fingerprints: "%s",\n  enabled: yes\n}\n\npfSense: {\n  url: "%s",\n  mirror_type: "srv",\n  signature_type: "fingerprints",\n  fingerprints: "%s",\n  enabled: yes\n}\n' \
            "$_CORE_URL" "$_FINGERPRINTS" "$_PKG_URL" "$_FINGERPRINTS" \
            > "$_NATIVE_CONF" \
            && msg_ok "Native pfSense repo config corrected: $_NATIVE_CONF" \
            || msg_wn "Could not write native repo config (non-critical)"

        # Verify DNS can resolve SRV records before proceeding
        msg_in "Checking DNS can resolve pkg.pfsense.org SRV records..."
        _SRV_CHECK="$(host -t srv _https._tcp.pkg.pfsense.org 2>/dev/null | grep 'SRV record')"
        if [ -n "$_SRV_CHECK" ]; then
            msg_ok "DNS SRV resolution working: $(echo "$_SRV_CHECK" | head -1 | awk '{print $NF}')"
        else
            msg_wn "DNS cannot resolve SRV records for pkg.pfsense.org"
            msg_wn "pkg update will likely fail. Fix DNS first:"
            printf "  System > General Setup > DNS Servers\n"
            printf "  Set to: 1.1.1.1 and 8.8.8.8\n"
            log "PKG: DNS SRV resolution failed"
        fi
        printf "\n"

        msg_in "Repo configuration:"
        printf "  Core URL     : %s\n" "$_CORE_URL"
        printf "  Pkg  URL     : %s\n" "$_PKG_URL"
        ...
        _CORE_CONF="$(printf 'pfSense-core: {\n  url: "%s",\n  mirror_type: "srv",\n  signature_type: "fingerprints",\n  fingerprints: "%s",\n  enabled: yes\n}\n' \
            "$_CORE_URL" "$_FINGERPRINTS")"
        _PKG_CONF="$(printf 'pfSense: {\n  url: "%s",\n  mirror_type: "srv",\n  signature_type: "fingerprints",\n  fingerprints: "%s",\n  enabled: yes\n}\n' \
            "$_PKG_URL" "$_FINGERPRINTS")"
        _FREE_CONF='FreeBSD: { enabled: no }'

        # Write to pkg standard location
        mkdir -p "$_REPO_DIR"
        printf '%s\n' "$_CORE_CONF" > "$_REPO_DIR/pfSense-core.conf" \
            && msg_ok "Written: $_REPO_DIR/pfSense-core.conf"
        printf '%s\n' "$_PKG_CONF"  > "$_REPO_DIR/pfSense.conf" \
            && msg_ok "Written: $_REPO_DIR/pfSense.conf"
        printf '%s\n' "$_FREE_CONF" > "$_REPO_DIR/FreeBSD.conf" \
            && msg_ok "Written: $_REPO_DIR/FreeBSD.conf"

        # Write to pfSense's own location (used by pfSense-upgrade and GUI updater)
        _PFS_REPO_DIR="/usr/local/share/pfSense/pkg/repos"
        if mkdir -p "$_PFS_REPO_DIR" 2>/dev/null; then
            printf '%s\n%s\n%s\n' "$_CORE_CONF" "$_PKG_CONF" "$_FREE_CONF" \
                > "$_PFS_REPO_DIR/pfSense-repo.conf" \
                && msg_ok "Written: $_PFS_REPO_DIR/pfSense-repo.conf"
        else
            msg_wn "Could not write to $_PFS_REPO_DIR (non-critical)"
        fi

        # Verify what was written — print first line of each conf so we can confirm
        printf "\n"
        msg_in "Verifying written configs (first url: line from each):"
        for _vf in \
            "$_REPO_DIR/pfSense-core.conf" \
            "$_REPO_DIR/pfSense.conf" \
            "$_PFS_REPO_DIR/pfSense-repo.conf"; do
            [ -f "$_vf" ] || continue
            _vurl="$(grep 'url:' "$_vf" 2>/dev/null | head -1 | tr -d ' ')"
            _vmirror="$(grep 'mirror_type:' "$_vf" 2>/dev/null | head -1 | tr -d ' ')"
            printf "  %-55s  %s  %s\n" "$(basename $_vf)" "$_vurl" "$_vmirror"
        done
        printf "\n"

        log "PKG REPO CONFIGS: rewritten for $_rv mirror=none"
...
_r_restore_files() {
    ...
    # Step 5: Clean temp PHP sessions
    printf "\n"
